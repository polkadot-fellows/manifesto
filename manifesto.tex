...
They should readily understand the difference between \textit{what something does} (i.e. the interface, the service offered and the reason for doing something) and \text{how it works} (i.e. the underlying data structures, implementation details, and the specific code). They should readily be able to grasp the solution a particular logic module is attempting to solve, asking the right questions as necessary and documenting their way to save others the trouble. They should be able to extrapolate intent and from it effectively review output and make insignhtful suggestions over improvements or additions which would help achieve the apparent goals.
